...
scala.io.Source.fromFile("manual1.tsv").mkString.split("\n+").map(_.trim).sorted.foreach{l =>
  ...
  val parent = syntax match {
    case "<br_style>"                    => "TypedAttr_BrStyle"
    case "<br_width>"                    => "TypedAttr_BrWidth"
    case "<color>"                       => "TypedAttr_Color"
    case "<br_width> <br_style> <color>" => "TypedAttr_BrWidthStyleColour"
  }
